<?php

declare(strict_types=1);

namespace App\Values\Props;

use App\Values\WpOrg\Author;
use Carbon\CarbonImmutable;
use Override;
use Spatie\LaravelData\Attributes\Validation\Between;
use Spatie\LaravelData\Attributes\Validation\Max;
use Spatie\LaravelData\Attributes\Validation\Min;
use Spatie\LaravelData\Attributes\Validation\RequiredWithout;
use Spatie\LaravelData\Attributes\Validation\Url;
use Spatie\LaravelData\Attributes\Validation\Uuid;
use Spatie\LaravelData\Optional;

final readonly class ThemeProps extends ModelProps
{
    /**
     * @param Optional|array<string, mixed> $tags
     * @param Optional|array<string, mixed>|null $ac_raw_metadata
     */
    public function __construct(
        #[Uuid]
        public Optional|string $id,
        #[Between(1, 255)]
        public string $slug,
        #[Between(1, 255)]
        public string $name,
        #[Between(1, 1024 * 128)]
        public Optional|string $description, // FIXME: should not be optional!

        #[Between(1, 32)]
        public string $version,
        #[Url]
        #[Max(1024)]
        public string $download_link,
        #[Between(1, 32)]
        public Optional|string|null $requires_php,
        public Optional|CarbonImmutable $last_updated,
        public Optional|CarbonImmutable $creation_time,
        #[Url]
        #[Max(1024)]
        public Optional|string|null $preview_url,
        #[Url]
        #[Max(1024)]
        public Optional|string|null $screenshot_url,
        #[Between(0, 100)]
        public Optional|int $rating,
        #[Min(0)]
        public Optional|int $num_ratings,
        #[Url]
        #[Max(1024)]
        public Optional|string|null $reviews_url,
        #[Min(0)]
        public Optional|int $downloaded,
        #[Min(0)]
        public Optional|int $active_installs,
        #[Url]
        #[Max(1024)]
        public Optional|string|null $homepage,
        public Optional|bool $is_commercial,
        #[Url]
        #[Max(1024)]
        public Optional|string|null $external_support_url,
        public Optional|bool $is_community,
        #[Url]
        #[Max(1024)]
        public Optional|string|null $external_repository_url,

        // associations
        #[RequiredWithout('author_id')]
        public Optional|Author $author,
        #[Uuid]
        #[RequiredWithout('author')]
        public Optional|string $author_id,
        public Optional|array $tags, // TODO (drop the column in the db too!)

        // AC-specific
        #[Between(1, 32)]
        public Optional|string $ac_origin,
        public Optional|CarbonImmutable|null $ac_created,
        public Optional|array|null $ac_raw_metadata,
    ) {}

    /**
     * @param array<string, mixed> $extra
     */
    public static function make(
        string $slug,
        string $name,
        string $version,
        string $description,
        string $download_link,
        array $extra = [],
    ): self {
        $args = [
            ...$extra,
            ...compact(
                'slug',
                'name',
                'version',
                'description',
                'download_link',
            ),
        ];
        return self::from($args);
    }

    #[Override] // empty override to narrow the return type (BaseData::from is not generic)
    public static function from(
        mixed ...$args,
    ): static {
        return parent::from(...$args);
    }
}
