...
})->skip(fn() => !config('app.aspire_press.api_authentication_enable'), 'API authentication is disabled');

it('should return 200 if the API authentication is disable', function () {
    $response = $this->getjson('/plugins/info/1.1?action=query_plugins');
    $response->assertStatus(200);
})->skip(fn() => config('app.aspire_press.api_authentication_enable'), 'API authentication is enabled');
